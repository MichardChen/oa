...
public class FileUploadCommand extends Command implements IPostCommand {
	...
	public void execute(final OutputStream out) throws ConnectorException {
		...
		try {
			String errorMsg = (this.errorCode == 0) ? "" : ErrorUtils
					.getInstance().getErrorMsgByLangAndCode(this.langCode,
	...
	private boolean fileUpload(final HttpServletRequest request) {
		try {
			...
			for (FileItem item : items) {
				if (!item.isFormField()) {
					String path = configuration.getTypes()
	...
	private boolean saveTemporaryFile(final String path, final FileItem item)
		...
		if (!ImageUtils.isImage(file)) {
			...
			//图片压缩
			Compress(path,this.newFileName,file);
			return true;
			...
			ImageUtils.createTmpThumb(item.getInputStream(), file,
					getFileItemName(item), this.configuration);
			if (FileUtils.checkFileSize(configuration.getTypes()
					.get(this.type), file.length())) {
				if (configuration.getEvents() != null) {
					configuration.getEvents().run(EventTypes.AfterFileUpload,
							args, configuration);
				}
	...
	private void Compress(String path,String photoName,File file) throws IOException{
	   ...
	   /* File newPath = new File(xkaFtpHistPath + "\\" + nsFaultXka.getInspDate());
	    ...
	    File newFile = new File(path + "\\" + photoName.substring(0,photoName.length()-4) + ".jpg");
	    // 压缩成jpg格式
	  	ImageIO.write(newBufferedImage, "jpg", newFile);
	  	// 如果非jpg,jpeg文件，则把源文件删除。注:JPG文件是采用替换的方式。
