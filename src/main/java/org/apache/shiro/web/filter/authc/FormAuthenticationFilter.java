...
public class FormAuthenticationFilter extends AuthenticatingFilter {
    ...
    protected boolean isLoginSubmission(ServletRequest request, ServletResponse response) {
	    // 2016-3-22 默认情况下，shiro只允许Post的方式登录，不允许GET方式。为了使微信回调(httpGet)时的使用的code和state时也可以登录，特别处理一下。
		String codeWeixin = request.getParameter("code");
		String state      = request.getParameter("state");
		if(StringUtils.isNotBlank(codeWeixin)&&StringUtils.isNotBlank(state)){
		   // 允许登录
		   return true;	
		}
		// 2016-3-22 以下为原先代码
