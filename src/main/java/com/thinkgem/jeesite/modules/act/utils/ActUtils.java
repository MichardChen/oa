...
//	}
	...
	public static String getFormUrl(String formKey, Act act){
		...
		if (StringUtils.isBlank(formServerUrl)){
			// 如果以static开头,代表使用html的页面。2016-5-27 added mlg.
			if(formKey.startsWith("/static") || formKey.startsWith("static")){
				if(formKey.startsWith("static")){
					formUrl.append("/");
				}
				// 20160723 对于手机上的菜单，增加版本号信息。如/static/nsmobile/pages/html/msgDetail.html时，
				// 转换成 /static/nsmobile/pages/html/msgDetail.html?v=20160723. 注:20160723是根据msgDetail.html到sys_menu中获取。
				formKey = formKey + "?v=" + getWxPageNewVersion(formKey);
			}
			else{ //否则为 Spring MVC的 jsp页面（Controller）。则增加/a的
				formUrl.append(Global.getAdminPath());
			}
		}else{ //从jeesite.propertis中获取地址。
	...
	/** 获取手机页面的最新版本
	 * 如/static/nsmobile/pages/html/msgDetail.html时，
				// 转换成 /static/nsmobile/pages/html/msgDetail.html?v=20160723. 注:20160723是根据msgDetail.html到sys_menu中获取。
	 * @param  formKey 如 /static/nsmobile/pages/html/msgDetail.html
	 * @return 版本号
	 */
	private static String getWxPageNewVersion(String formKey){
		String version = "20160723";
		
		//1)获取页面的文件名 e.g msgDetail.html 
		int pagePos = formKey.lastIndexOf("/");
		if(pagePos < 0){
			return version;
		}
		String pageName = formKey.substring(pagePos + 1);	
		//2)根据页面名(msgDetail.html),获取版本号(格式20160723)。
	    version = UserUtils.getWxPageVersion(pageName);
	...
	public static UserEntity toActivitiUser(User user){
		...
		// u_ + 用户id 2016
		userEntity.setId("u_"+user.getId());
