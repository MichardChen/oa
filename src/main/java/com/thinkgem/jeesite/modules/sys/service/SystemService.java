...
public class SystemService extends BaseService implements InitializingBean {
	...
	/**
	 * 2016-1-19 根据微信openId获取用户
	 * @param openId
	 * @return
	 */
	public User getUserByOpenId(String openId) {
		return UserUtils.getByOpenId(openId);
	}
	
	/**
	 * 2016-7-2 根据微信企业号的userId获取用户
	 * @param 
	 * @return
	 */
	public User getByWxUserId(String wxUserId){
		return UserUtils.getByWxUserId(wxUserId);
	}
	...
	public void saveUser(User user) {
		if (StringUtils.isBlank(user.getId())){
			user.preInsert();
			userDao.insert(user);
			// Insert后，根据自增长id来获取实际表ID,设置到user上。201605017
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	 ...
	 * 绑定（解绑定）微信openId
	 * @param id:用户的userid
	 * @param openid:绑定时,传非空值;解绑时,传空值。
	 * */
	@Transactional(readOnly = false)
	public void updateUserOpenId(String id, String openId) {
		User user = new User(id);
		// 保存上次登录信息
		user.setWechatOpenId(openId);
		userDao.updateUserOpenId(user);
		UserUtils.clearCache(user);
	}
	 ...
	 * */
	@Transactional(readOnly = false)
	public void updateWxAccessToken(String id, String wxAccessToken) {
		User user = new User(id);
		// 保存上次登录信息
		user.setWxAccessToken(wxAccessToken);
		userDao.updateWxAccessToken(user);
		UserUtils.clearCache(user);
	}
	
	/**
	 * 绑定（解绑定）到微信企业号的Userid
	 * @param id:用户的userid
	 * @param wxUserId:绑定时,传非空值;解绑时,传空值。
	 * */
	@Transactional(readOnly = false)
	public void updateWxqyUserId(String id, String wxUserId) {
		User user = new User(id);
		user.setWxUserId(wxUserId);
		userDao.updateWxqyUserId(user);
		UserUtils.clearCache(user);
	}
	...
	public void saveRole(Role role) {
		if (StringUtils.isBlank(role.getId())){
			...
			// Insert后，根据自增长id来获取实际表ID,设置到user上。201605017
			RoleModel roleModel = roleDao.findByAutoIncId(role.getAutoIncId());
		...
		// 清除缓存。
		CacheUtils.remove(RoleUtils.ROLE_CACHE, RoleUtils.ROLE_CACHE_ID_+ role.getId());
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public void saveMenu(Menu menu) {
		...
		if (StringUtils.isBlank(menu.getId())){
			...
			// 2016-06-30 必须获取真正的id,然后重新设置到entity中。
			MenuModel menuModel = menuDao.findByAutoIncId(menu.getAutoIncId());
		...
		// 清除用户菜单缓存
		UserUtils.removeCache(UserUtils.CACHE_MENU_LIST);
		// 清除菜单缓存(所有菜单)。注意：此缓存不跟登录session有关。
		CacheUtils.remove(UserUtils.CACHE_MENU_LIST_ALL);
//		// 清除权限缓存
//		systemRealm.clearAllCachedAuthorizationInfo();
		// 清除日志相关缓存
