...
public class Page<T> {
	...
	final public static int MAX_PAGE_SIZE = 1000000;
	final public static int DEFAULT_PAGE_SIZE = 100;
	final public static String DEFAULT_CHARSET = "gbk";
	...
	/**
	 * 构造方法(JQGrid用)。20151104
	 * */
	public Page(HttpServletRequest request, boolean isPagination) {
		...
		if (isPagination) {
			// 分页
			pageNo = Integer.parseInt(request.getParameter("page") == null ? "1" : request.getParameter("page"));
			pageSize = Integer.parseInt(request.getParameter("rows") == null ? (DEFAULT_PAGE_SIZE + "") : request.getParameter("rows"));
		} else {
			// 不分页
			pageNo = 0;
		...
		if (StringUtils.isNotBlank(request.getParameter("sidx"))) {
			String sorttype = request.getParameter("stype");
			if (StringUtils.equalsIgnoreCase(sorttype, "number")) {
				orderBy += request.getParameter("sidx") + "+0";
			} else if (StringUtils.equalsIgnoreCase(sorttype, "text")) {
				orderBy += "CONVERT(" + request.getParameter("sidx") + " USING latin1)";
			} else if (StringUtils.equalsIgnoreCase(sorttype, "gbk")) {
				orderBy += "CONVERT(" + request.getParameter("sidx") + " USING " + DEFAULT_CHARSET + ")";
			} else {
...
//	}
	...
	public int getTotalPage() {
		int totalPage = (int) (this.count / this.pageSize);
		if (count % pageSize > 0) {
			totalPage++;
		}
		return totalPage;
