...
public class ImportExcel {
	 ...
	 * 本workBook中的sheet列表。 
	 * */
	private List<DictModel> sheetList = new ArrayList<DictModel>();
	/**
	 * 构造函数
	 ...
	 * 获取所有的sheet名和index 
	 * */
	public List<DictModel> getSheetList(){
		...
		for (int i = 0; i < this.wb.getNumberOfSheets(); i++) {
			...
			model.setLabel(this.sheet.getSheetName());
			dictList.add(model);
		}
		...
		try{
			...
			if (cell != null){
				if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC){
					...
					// 2016-1-13 对于Excel公式值的获取。如 =E2*E3  = 'file1.xls'!A12 
					try {
	...
	public <E> List<E> getDataList(Class<E> cls, int... groups) throws InstantiationException, IllegalAccessException{
		...
		// 2015-12-26 mlg. 修改bug,解决最后一行没导入的问题。
		// i < this.getLastDataRowNum() 改成 i <= this.getLastDataRowNum()
