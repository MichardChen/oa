    ...
    private static void doRequest(final RequestMethod method, final String url,
        ...
        switch (method) {
            case GET:
				...
				URL urlTemp = null;
				try {
					urlTemp = new URL(getUrl);
				} catch (MalformedURLException e1) {
					e1.printStackTrace();
				}
				URI uriTarget = null;
				try {
					uriTarget = new URI(urlTemp.getProtocol(), urlTemp.getHost(), urlTemp.getPath(), urlTemp.getQuery(), null);
				} catch (URISyntaxException e1) {
					e1.printStackTrace();
				}
                // 
                request = new HttpGet(uriTarget);
            ...
            case POST:
                ...
                // 使用
				URL urlTempPost = null;
				try {
					urlTempPost = new URL(url);
				} catch (MalformedURLException e1) {
					e1.printStackTrace();
				}
				URI uriTargetPost = null;
				try {
					uriTargetPost = new URI(urlTempPost.getProtocol(), urlTempPost.getHost(), urlTempPost.getPath(), urlTempPost.getQuery(), null);
				} catch (URISyntaxException e1) {
					e1.printStackTrace();
				}
				// 
                request = new HttpPost(uriTargetPost);
                //上传文件
