...
//警告对话框 2016/4/1添加(输入框出现验证错误提示，inputId：输入框的ID)
function alerts(mess,inputId,closed){
	$.jBox.info(mess, '提示', {closed:function(){
		if (typeof closed == 'function') {
			closed();
		}
...
function promptx(title, lable, href, closed){
	$.jBox("<div class='form-search' style='padding:20px;text-align:center;'>" + lable + "：<input type='text' id='txt' name='txt'/></div>", {
		...
		if (typeof href == 'function') {
			...
			resetTip(); //loading();
			location = href + encodeURIComponent(encodeURIComponent(f.txt));
		}
	},closed:function(){
		if (typeof closed == 'function') {
			closed();
