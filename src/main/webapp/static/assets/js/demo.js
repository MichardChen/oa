...
var Demo = function() {
   ...
   var runDemoSettings = function() {
      ...
      if ($('#skin-toolbox').length) {
         ...
         // 2015-12-29 mlg. 页面的导航条的背景默认改为 bg-alert.方法：替换 bg-light成 bg-alert。
         // Theme Settings
         var settingsObj = {
                  // 'headerTone': true,
                  'headerSkin': 'bg-alert',
            ...
            $.each(settingsParse, function(i, e) {
               switch (i) {
                  case 'headerSkin':
                     ...
                     $('#toolbox-header-skin input[value="bg-alert"]').prop('checked', false);
                     $('#toolbox-header-skin input[value="' + e + '"]').prop('checked', true);
                     break;
                  case 'sidebarSkin':
                     Sidebar.removeClass(sidebarSkins).addClass(e);
                     $('#toolbox-sidebar-skin input[value="bg-alert"]').prop('checked', false);
         ...
         $('#toolbox-header-skin input').on('click', function() {
            ...
            // 20151220 增加 start。选中颜色后，关闭换肤的div。
            $("#skinLi").removeClass("open");
